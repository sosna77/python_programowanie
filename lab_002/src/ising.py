...
def visualize(history, magnetization, energy):
    ...
    im = ax.imshow(history[0], cmap='grey', vmin=-1, vmax=1)
...
def main():
    ...
    parser.add_argument('-B', '--field', type=float, default=0.0, help='external magnetic field (default=0.0)')
    parser.add_argument('-M', '--steps', type=int, default=1000, help='number of macrosteps (default=1000)')
    ...
    history = np.empty((M,N,N), dtype=np.int8)
    magnetization = np.empty(M, dtype=np.float32)
    energy = np.empty(M, dtype=np.float32)
    ...
    print(f'INITIAL SIMMULATION TOOK: {sim_end - start_time:.4f} s')
